...
public class Display : DisplayDevice
{
    ...
    public override string DisplayName
    {
        get
        {
            if (IsValid)
            {
                var displayAdapters = DisplayAdapter.GetDisplayAdapters();
                var displayDevices = displayAdapters
                    .SelectMany(adapter => adapter.GetDisplayDevices(base.IsAvailable));
                var displayDevice = displayDevices
                    .FirstOrDefault(
                        device => device.DevicePath.Equals(DevicePath) && device.DeviceKey.Equals(DeviceKey));
                return displayDevice?.DisplayName;
            }
